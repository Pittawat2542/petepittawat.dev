...
const CardMetaRowComponent: FC<CardMetaRowProps> = ({ children, className }) => (
  <div
    className={cn(
      '-my-1 flex max-w-full min-w-0 flex-nowrap items-center gap-2 overflow-hidden py-1',
      className
    )}
  >
    {children}
...
function MeasuredOverflowRowComponent<TItem>({
  ...
  const updateVisibleCount = useCallback(() => {
    ...
    const gap = getGapWidth(container);
    ...
    for (let candidate = maxCount; candidate >= floorCount; candidate -= 1) {
      ...
      const totalGap = Math.max(0, visibleParts - 1) * gap;

      if (itemWidth + overflowWidth + totalGap <= availableWidth || candidate === floorCount) {
  ...
  }, [floorCount, items.length, maxCount]);
    ...
    >
      {visibleItems.map((item, index) => (
        <span
          ...
          className={cn('inline-flex min-w-0 shrink', itemClassName)}
        >
      ...
      <div
        ref={measureRef}
        aria-hidden="true"
        className="pointer-events-none invisible absolute top-0 left-0 -z-10 flex max-w-none flex-nowrap items-center gap-2 overflow-visible"
      >
...
const CardTagListComponent: FC<CardTagListProps> = ({
  ...
  const renderTag = (tag: string) => (
    <Badge
      className={cn(
        'type-caption max-w-[12rem] min-w-0 rounded-full border px-3 py-1 font-medium md:text-xs',
  ...
  if (typeof maxVisible === 'number' && onOverflowClick) {
    return (
      <MeasuredOverflowRow
        ...
        className="-my-1 flex-nowrap py-1"
      />
    );
  }
