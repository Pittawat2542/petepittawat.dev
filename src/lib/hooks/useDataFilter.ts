...
export interface FilterOptions<T> {
  ...
  readonly filterFields?: Readonly<Record<string, (item: T) => string | number | boolean | readonly (string | number | boolean)[]>>;
}
...
export function useDataFilter<T>(
  ...
  const setQ = useCallback((query: string) => {
    ...
    if (query.length > COMPONENT_CONFIG.FILTER.MAX_SEARCH_LENGTH) {
      console.warn(`Search query truncated to ${COMPONENT_CONFIG.FILTER.MAX_SEARCH_LENGTH} characters`);
